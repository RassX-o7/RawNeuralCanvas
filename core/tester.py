...
class Tester:
    ...
    def testing(self):
        ...
        for index in np.random.permutation(self.test_size):
            ...
            # print(true_label, type(true_label), np.shape(true_label))

            self.cm_true[true_label.item()][prediction]+=1 # imp since after .get() vectorized batch size 1 returns 1d array 
            self.cm_ideal[true_label.item()][true_label]+=1
